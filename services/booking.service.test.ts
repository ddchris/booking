...
describe('BookingService', () => {
  ...
  describe('createBooking', () => {
    it('should successfully create a booking when slot is free', async () => {
      // Setup
      ...
      // Verify Writes
      expect(mockTransaction.set).toHaveBeenCalledTimes(2) // booking + public_slots
      expect(mockTransaction.update).toHaveBeenCalledTimes(1) // user

      // Verify User Update (activeBookingTimeSlot set)
      const userUpdateCall = mockTransaction.update.mock.calls[0]
      expect(userUpdateCall[1].activeBookingTimeSlot).toBe(timeSlot)
    })

    it('should throw error if slot is already taken', async () => {
      mockTransaction.get.mockImplementation((ref) => {
        if (ref.path.includes('public_slots')) {
          // Slot exists (taken)
          return { exists: () => true }
  ...
  describe('cancelBooking', () => {
    it('should cancel booking if limits valid', async () => {
      ...
      // Valid Slot: Time + 3 hours
      const slotTime = 1700000000000 + (3 * 60 * 60 * 1000)
